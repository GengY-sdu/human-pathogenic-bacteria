...
load("pathogen/map_spatial_cv_abun/HPB-500_abun.Rdata")
...
load("pathogen/map_spatial_cv_abun/HPB-800_abun.Rdata")
grid_search <- data.table(custom$results)
grid_search$'ntree' <- rep(800,nrow(grid_search))
final_grid <- dplyr::bind_rows(final_grid,grid_search)

load("pathogen/map_spatial_cv_abun/HPB-1000_abun.Rdata")
grid_search <- data.table(custom$results)
grid_search$'ntree' <- rep(1000,nrow(grid_search))
final_grid <- dplyr::bind_rows(final_grid,grid_search)

load("pathogen/map_spatial_cv_abun/HPB-1200_abun.Rdata")
grid_search <- data.table(custom$results)
...
load("pathogen/map_spatial_cv_abun/HPB-2000_abun.Rdata")
grid_search <- data.table(custom$results)
grid_search$'ntree' <- rep(2000,nrow(grid_search))
final_grid <- dplyr::bind_rows(final_grid,grid_search)

load("pathogen/map_spatial_cv_abun/HPB-2500_abun.Rdata")
grid_search <- data.table(custom$results)
grid_search$'ntree' <- rep(2500,nrow(grid_search))
final_grid <- dplyr::bind_rows(final_grid,grid_search)

write.csv(final_grid,file = 'pathogen/map_spatial_cv_abun/grid_cv_abun.csv',row.names = F)
...
write.csv(plot,file = 'pathogen/map_spatial_cv_abun/train_cv_abun.csv',row.names = F)
...
write.csv(plot,file = 'pathogen/map_spatial_cv_abun/test_cv_abun.csv',row.names = F)
