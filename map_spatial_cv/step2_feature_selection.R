...
save(inTrain,file = 'pathogen/map_spatial_cv/inTrain.Rdata')
trainx <- x[inTrain, ]
testx <- x[-inTrain, ]
...
save(sb1,file = 'pathogen/map_spatial_cv/sb1.Rdata')
...
stopCluster(cl)
save(rfFuncs,file = 'pathogen/map_spatial_cv/rfFuncs.Rdata')
...
save(optvar,file = 'pathogen/map_spatial_cv/optvar.Rdata')
write.csv(sum,file = 'pathogen/map_spatial_cv/feature_cv.csv',row.names = F)
