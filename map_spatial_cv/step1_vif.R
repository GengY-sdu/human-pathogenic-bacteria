...
df <- read.csv(file = "pathogen/dat.csv",header = T)
df <- na.omit(df)
colnames(df)[2] <- "longitude"
colnames(df)[3] <- "latitude"
form_all2 <- as.formula(mean_otu_diversity~.)
impVars <- df[,-1]
...
if (class(all_vifs_test) == "try-error"){
  ...
  form_all_new <- as.formula(paste("mean_otu_diversity ~ ", 
...
if(any(all_vifs > 10)){
  ...
  while((nrow(all_vifs) > 2)& (max(all_vifs[, 1]) > 10)  &  
    ...
    fullForm <- paste ("mean_otu_diversity ~ ", paste (names(impVars), collapse=" + "), sep="")  
  ...
  all_vifs <- as.data.frame(all_vifs)
  vif_filtered_variables <- rownames(all_vifs)[!names(all_vifs) %in% "mean_otu_diversity"]
}

df_filter <- df[,c("mean_otu_diversity",vif_filtered_variables)]

save(df_filter,file = "pathogen/map_spatial_cv/ml_df.Rdata")
