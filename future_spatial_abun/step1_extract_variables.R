...
total <- pblapply(historial_clim,function(x){
  df_0 <- data.table::fread("pathogen/df_412_terra_nohuman.csv",integer64 = "numeric")
  n1 <- raster::raster(x)
...
df0 <- data.table::fread("pathogen/df_412_terra_nohuman.csv",integer64 = "numeric")
mapped_df <- cbind(df0,mapped_df)
rm(df0,cl,total,num_cores)
df0_add <- data.table::fread("pathogen/df0_add_nohuman.csv",integer64 = "numeric")
mapped_df <- df0_add[mapped_df,on = c("lat_lon1")]
fwrite(mapped_df,"pathogen/glm_start_table_nohuamn.csv")
