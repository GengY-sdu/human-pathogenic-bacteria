...
save(inTrain,file = 'pathogen/map_remove_abun/inTrain.Rdata')
save(inTest,file = 'pathogen/map_remove_abun/inTest.Rdata')
trainx <- x[inTrain, ]
testx <- x[inTest, ]
...
stopCluster(cl)
save(rfFuncs,file = 'pathogen/map_remove_abun/rfFuncs_abun.Rdata')
...
save(optvar,file = 'pathogen/map_remove_abun/optvar.Rdata')
write.csv(sum,file = 'pathogen/map_remove_abun/feature_set_abun.csv',row.names = F)
