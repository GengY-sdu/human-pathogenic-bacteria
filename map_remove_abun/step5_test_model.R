...
load("pathogen/map_remove_abun/HPB-500_abun.Rdata")
...
load("pathogen/map_remove_abun/HPB-800_abun.Rdata")
grid_search <- data.table(custom$results)
...
load("pathogen/map_remove_abun/HPB-1200_abun.Rdata")
grid_search <- data.table(custom$results)
grid_search$'ntree' <- rep(1200,nrow(grid_search))
final_grid <- dplyr::bind_rows(final_grid,grid_search)

load("pathogen/map_remove_abun/HPB-1500_abun.Rdata")
grid_search <- data.table(custom$results)
grid_search$'ntree' <- rep(1500,nrow(grid_search))
final_grid <- dplyr::bind_rows(final_grid,grid_search)

load("pathogen/map_remove_abun/HPB-2000_abun.Rdata")
grid_search <- data.table(custom$results)
grid_search$'ntree' <- rep(2000,nrow(grid_search))
final_grid <- dplyr::bind_rows(final_grid,grid_search)

load("pathogen/map_remove_abun/HPB-2500_abun.Rdata")
grid_search <- data.table(custom$results)
grid_search$'ntree' <- rep(2500,nrow(grid_search))
final_grid <- dplyr::bind_rows(final_grid,grid_search)

write.csv(final_grid,file = 'pathogen/map_remove_abun/grid_set_abun.csv',row.names = F)
...
write.csv(plot,file = 'pathogen/map_remove_abun/train_set_abun.csv',row.names = F)
...
write.csv(plot,file = 'pathogen/map_remove_abun/test_set_abun.csv',row.names = F)
