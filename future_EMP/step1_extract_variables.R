...
total <- pblapply(historial_clim,function(x){
  df_0 <- data.table::fread("pathogen/future_EMP/df_412_terra_nohuman.csv",integer64 = "numeric")
  n1 <- raster::raster(x)
...
df0 <- data.table::fread("pathogen/future_EMP/df_412_terra_nohuman.csv",integer64 = "numeric")
mapped_df <- cbind(df0,mapped_df)
rm(df0,cl,total)
df0_add <- data.table::fread("pathogen/future_EMP/df0_add_nohuman.csv",integer64 = "numeric")
mapped_df <- df0_add[mapped_df,on = c("lat_lon1")]
fwrite(mapped_df,"pathogen/future_EMP/glm_start_table_nohuamn_EMP.csv")
